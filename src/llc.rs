...
// --- Constants for Control Field Identification ---
/// Mask to check the LSB for I-format.
const CONTROL_LSB_I_FORMAT_MASK: u8 = 0x01;
/// Pattern for I-format (LSB is 0).
const CONTROL_LSB_I_FORMAT_PATTERN: u8 = 0x00;

/// Mask to check the two LSBs for S-format or U-format.
const CONTROL_LSB_SU_FORMAT_MASK: u8 = 0x03;
/// Pattern for S-format (LSBs are 01).
const CONTROL_LSB_S_FORMAT_PATTERN: u8 = 0x01;
/// Pattern for U-format (LSBs are 11).
const CONTROL_LSB_U_FORMAT_PATTERN: u8 = 0x03;
...
impl LLC {
    ...
    #[inline]
    pub fn dsap_is_group_address(&self) -> bool {
        (self.dsap & 0x01) != 0
    }
    ...
    #[inline]
    pub fn set_dsap(&mut self, address: u8, is_group: bool) {
        self.dsap = ((address & 0x7F) << 1) | (if is_group { 0x01 } else { 0x00 });
    }
    ...
    pub fn set_ssap(&mut self, address: u8, is_response: bool) {
        self.ssap = ((address & 0x7F) << 1) | (if is_response { 0x01 } else { 0x00 });
    }

    /// Determines the LLC PDU frame type based on the control field's first byte.
    #[inline]
    pub fn frame_type(&self) -> LlcFrameType {
        if (self.ctrl[0] & CONTROL_LSB_I_FORMAT_MASK) == CONTROL_LSB_I_FORMAT_PATTERN {
            LlcFrameType::I
        } else if (self.ctrl[0] & CONTROL_LSB_SU_FORMAT_MASK) == CONTROL_LSB_S_FORMAT_PATTERN {
            LlcFrameType::S
        } else if (self.ctrl[0] & CONTROL_LSB_SU_FORMAT_MASK) == CONTROL_LSB_U_FORMAT_PATTERN {
            LlcFrameType::U
        } else {
...
mod tests {
    ...
    fn test_dsap_methods() {
        ...
        assert_eq!(llc.dsap_address(), 0x42);
        assert!(!llc.dsap_is_group_address());
        assert_eq!(llc.dsap, (0x42 << 1) | 0x00); // 0x84

        // Test group address
        llc.set_dsap(0x7F, true); // Max 7-bit address, Group
        assert_eq!(llc.dsap_address(), 0x7F);
        assert!(llc.dsap_is_group_address());
